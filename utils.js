...
export async function fetchTunes(stream, filter) {
  ...
  if (jsonTunes && jsonTunes.length > 0) {
    ...
    for (const tune of jsonTunes) {
      ...
      var tuneContent = `- ${tune.year} ${tune.make} ${tune.model} (${tune.class} ${tune.pi}, ${tune.drivetrain}, ${tune.bestfor.map(item => capitalize(item)).join(' / ') }${tune.antilag === 'Yes' ? ', with anti-lag' : ''}${tune.comments !== '' ? ', ' + tune.comments : ''}) by ${tune.author}: ${tune.sharecode}\n`;
